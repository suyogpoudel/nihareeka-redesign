...
const buttonClassNames =
  "relative flex size-(--cell-size) text-base sm:text-sm items-center justify-center rounded-lg text-foreground disabled:pointer-events-none disabled:opacity-64 [&_svg]:pointer-events-none [&_svg]:shrink-0";
...
function Calendar({
  ...
  const defaultClassNames = {
    ...
    dropdown_root:
      "relative has-focus:border-ring has-focus:ring-ring/50 has-focus:ring-[3px] border border-input shadow-xs/5 rounded-lg px-[calc(--spacing(3)-1px)] h-9 sm:h-8 [&_svg]:opacity-80 [&_svg]:size-4.5 sm:[&_svg]:size-4 [&_svg]:pointer-events-none [&_svg]:-me-1",
    dropdowns:
  ...
  const defaultComponents = {
    Chevron: ({
      ...
      if (orientation === "left") Icon = ChevronLeftIcon;
      else if (orientation === "right") Icon = ChevronRightIcon;
